...
function Keywords({ id, type = "movie" }) {
  const { data, isPending } = useMovieDB(type, id, "keywords");
      ...
      <ul className="flex flex-wrap gap-1">
        ...
        {isPending
          ...
          : keywords?.map((keyword, i) => (
              <li
                key={i}
                className="2xl: border-grey-primary hover:border-primary cursor-pointer rounded-md border-1 px-1.5 py-0.5 text-xs duration-300 md:px-1.5 lg:py-1"
              >
