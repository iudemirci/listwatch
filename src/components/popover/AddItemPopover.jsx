...
function AddItemPopover() {
  ...
  const type =
    (item?.type !== "Scripted" && item?.type) ||
    (item?.release_date || item?.release_date === "" ? "movie" : "tv");
    ...
    addItem(
      { listName: listName, item: savedItem },
      {
        onSuccess: () => {
          ...
          queryClient.invalidateQueries(["user", "lists", "personal"]);
      ...
      {isAddItemOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-grey-tertiary border-grey-secondary absolute bottom-0 mx-4 flex w-full max-w-90 flex-col gap-2.5 rounded-xl border-1 p-4 sm:max-w-110 lg:max-w-140"
          >
            <div className="bg-grey-secondary/70 flex gap-4 rounded-lg p-2">
              ...
              <div className="flex flex-1 flex-col justify-between">
                <div className="flex flex-1 flex-col gap-1.5">
                  ...
                  {listName !== "Watchlist" && listName !== "" && !isCreate && (
                    <StarRating
                      size={1.2}
            ...
            {!isCreate && (
              <div
                className="bg-grey-secondary/70 hover:bg-grey-secondary cursor-pointer rounded-lg p-2 duration-300"
            ...
            {!isCreate && (
              <div className="flex flex-col gap-1">
                <Title level={5}>Select a list</Title>
                <ul className="divide-grey-primary/50 flex max-h-50 flex-col divide-y overflow-y-auto rounded-lg">
                  {!isCreate &&
                    lists?.map((list) => (
                      <li
                        key={list?.id}
            ...
            {isCreate && (
              <div className="bg-grey-secondary/70 flex flex-col gap-2 rounded-lg p-2">
                <input
